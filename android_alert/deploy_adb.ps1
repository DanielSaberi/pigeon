...
& adb -s $AndroidSerial push (Join-Path $ScriptDir "update_receiver_termux.sh") "$RemoteDir/update_receiver_termux.sh"
& adb -s $AndroidSerial push (Join-Path $ScriptDir "update_receiver_termux.sh") "/sdcard/a"
...
Write-Host ""
Write-Host "For one-command update/install from Termux:"
Write-Host "  sh /sdcard/a"
