...
Usage (OpenRouter):
    python benchmark_vlm.py --api-key YOUR_KEY --coco-dir data/coco --output-dir results

Usage (local llama-server):
    python benchmark_vlm.py --base-url http://localhost:8080/v1 --model qwen3.5-35b \
        --coco-dir data/coco --output-dir results
"""
import argparse
import base64
import json
...
def classify_image(client, model, image_path, no_think=False):
...
def main():
    parser = argparse.ArgumentParser(
        description="COCO bird classification benchmark using Vision LLM",
    ...
    parser.add_argument("--model", default="qwen/qwen3-vl-235b-a22b-thinking",
                        help="Model name/slug")
    parser.add_argument("--coco-dir", required=True, help="Path to COCO dataset")
    parser.add_argument("--output-dir", default="results", help="Output directory")
    parser.add_argument("--num-negative", type=int, default=125,
                        help="Number of non-bird images to test for FP rate")
    parser.add_argument("--limit", type=int, default=0,
                        help="Limit total images (0 = no limit). Splits evenly between positive/negative.")
    ...
    # Get bird images (all) + random non-bird images
    ...
    if args.limit > 0:
        n_pos = min(len(bird_img_ids), args.limit // 2)
        n_neg = args.limit - n_pos
        ...
        sampled_neg = random.sample(non_bird_ids, min(args.num_negative, len(non_bird_ids)))
        test_img_ids = sorted(bird_img_ids) + sampled_neg

    images = coco_gt.loadImgs(test_img_ids)

    # Derive a short name for file outputs
    ...
    print(f"Testing {len(images)} images: {n_pos_test} with birds + {n_neg_test} without")
    print(f"Model: {args.model}")
    print(f"API:   {args.base_url}")
    print()

    # Check for cached results
    cache_path = os.path.join(args.output_dir, f"vlm_{model_slug}_results.json")
    ...
    for i, img_info in enumerate(images):
        ...
        image_path = os.path.join(args.coco_dir, "val2017", img_info["file_name"])
        if not os.path.exists(image_path):
            continue
        has_bird_gt = img_info["id"] in bird_img_ids

        result = classify_image(client, args.model, image_path, no_think=args.no_think)

        if result is None:
            parse_failures += 1
            # Treat parse failure as "no bird" with low confidence
    ...
    # Save metrics
    metrics_path = os.path.join(args.output_dir, f"vlm_{model_slug}_metrics.json")
